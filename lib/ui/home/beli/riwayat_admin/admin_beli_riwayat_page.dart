...
import 'package:tokomobil/bloc/admin/admin_transaksi/admin_transaksi_bloc.dart';
...
class AdminBeliRiwayatPage extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocProvider(
        create: (context) =>
            AdminTransaksiBloc()..add(AdminTransaksiGetListEvent()),
        child: AdminBeliRiwayatView(),
      ),
    );
  }
}

class AdminBeliRiwayatView extends StatelessWidget {
  AdminTransaksiBloc bloc;
  RefreshController _refreshController =
      RefreshController(initialRefresh: false);
  void _onRefresh() async {
    // monitor network fetch
    await Future.delayed(Duration(milliseconds: 1000));
    bloc..add(AdminTransaksiGetListEvent(refresh: true));
  ...
  void _onLoading() async {
    // monitor network fetch
    await Future.delayed(Duration(milliseconds: 1000));
    bloc..add(AdminTransaksiGetListEvent());
    _refreshController.loadComplete();
  }

  @override
  Widget build(BuildContext context) {
    bloc = BlocProvider.of<AdminTransaksiBloc>(context);
    return Container(
      color: Theme.of(context).scaffoldBackgroundColor,
      child: BlocConsumer<AdminTransaksiBloc, AdminTransaksiState>(
        listener: (context, state) {
          if (state is AdminTransaksiStateError) {
            ToastUtil.error(message: state.errors['message'] ?? '');
          } else if (state is AdminTransaksiStateSuccess) {
            ToastUtil.success(message: state.data['message'] ?? '');
          }
        },
        builder: (context, state) {
          if (state is AdminTransaksiListLoaded) {
            AdminTransaksiListLoaded stateData = state;
            if (stateData.transaksis != null &&
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: ListTile(
                          title: Text("${transaksi['user_nama']}"),
                          subtitle: RichText(
                            text: TextSpan(children: [
                              TextSpan(
                                  text:
                                      "Tanggal: ${DateTimeUtil.humanize(transaksi['transaksi_created_at'])}\n",
              ...
              return ErrorState(
                title: "User belum pernah melakukan transaksi",
                onTap: () {
                  bloc..add(AdminTransaksiGetListEvent(refresh: true));
                },
              );
